"""TelecomBackup 候选完整性校验（借鉴 CVRP validator，简化版）。

在评分前以"可执行检查"强制任务约束（不只是自然语言）：
  1. EVOLVE-BLOCK 完整性：标记必须存在；标记外的代码必须与初始 baseline 逐字节一致。
  2. 禁引用：候选不得 import verification 的评测/生成模块（evaluate/generator），
     不得出现绝对路径，不得按实例名硬编码调度。
  3. 确定性：同一实例跑两次必须输出一致（调度器需要可复现）。
  4. candidate_env：候选子进程环境剥离 FRONTIER_*/TELECOM_EVAL_* 变量，
     封死"通过宿主环境变量定位评测基线"的侧信道。

候选允许 import `verification/simulator.py`（白盒计分器，任务有意暴露），
但不允许 import 评测/生成逻辑。纯标准库。
"""
...
FORBIDDEN_RE = (
    re.compile(r"verification[\\/](?:evaluate|generator)"),
    re.compile(r"verification\s*\.\s*(?:evaluate|generator)\b"),
    re.compile(r"\b(?:from|import)\s+(?:evaluate|generator)\b"),
)
# Windows 盘符 / POSIX 家目录绝对路径。
ABS_PATH_RE = re.compile(r"[A-Za-z]:[\\/]|/home/|/Users/")
# 按实例名硬编码调度，如 "instance_1": [...] 或 "gen_42_1": [...]
...
def fixed_region(parts: tuple[str, str, str]) -> str:
    """EVOLVE-BLOCK 之外的只读部分。

    行尾归一化：CRLF/LF 差异与结尾缺换行（保存/复制时的常见无损差异）不计为
    "修改了固定区"——固定区是否被改看内容，不看行尾。
    """
...
def static_check_source(src: str, baseline_src: str | None = None) -> list[str]:
    ...
    for pat in FORBIDDEN_RE:
        if pat.search(src):
            issues.append("candidate references forbidden evaluation/generation module")
...
def candidate_env() -> dict[str, str]:
    ...
    for key in list(env):
        upper = key.upper()
        if upper.startswith("FRONTIER") or upper.startswith("TELECOM_EVAL"):
            del env[key]
    return env
