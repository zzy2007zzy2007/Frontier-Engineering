...
一根连续浇铸的钢坯被拉过切割机。**结晶器异常会产生 0.8m 报废段，但 agent 只在报废段距切割线 `reveal_lead` 米以内时才被告知**（默认 8m → 隐藏异常；`reveal_lead=60` 为"忠实版"，此时 agent 能看到所有相关异常）。agent **每个切割决策点被调用一次、只拿到当下可见状态**，永远看不到未来，并要选下一刀长度。最后按**完整隐藏报废表**评分：任何与报废段重叠的切块都被污染、整块报废。
...
> 诚实说明：agent 改进呈**阶梯式跳变**（卡 baseline 多代、某次突变到 ~68-72），不是渐进爬升
> ——符合"强约束下把策略一次想对"胜过增量搜索。
